...
int ffmpeg_do_decode(mp_decoder_format_t mediaFormat, char *mediaFileIn, char *mediaFileOut)
{
	AVCodec *pCodec;
	...
	if (mediaFormat == kMpDecoderFormat_H264)
	{
		codec_id = AV_CODEC_ID_H264;
	}
	else if (mediaFormat == kMpDecoderFormat_HEVC)
	{
		codec_id = AV_CODEC_ID_HEVC;
	}
	else if (mediaFormat == kMpDecoderFormat_MPEG2)
	{
		codec_id = AV_CODEC_ID_MPEG2VIDEO;
	}
	char *filepath_in = mediaFileIn;

	//char filepath_out[] = "bigbuckbunny_480x272.yuv";
	char *filepath_out = mediaFileOut;
	int first_time = 1;
